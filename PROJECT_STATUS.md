...
- **Sprint 15:**
  ...
  - Verified on device (1789-song library): the detector initially suggested excluding the SD card's music container (`…/Recordings (1)`, which recursively holds 1,775 songs) — excluding it wiped the library to 14 songs, which exposed the false positive. The recursive music-container guard fixed it: the wizard now suggests only the genuine recording folders (12 internal + 4 SD card files), **Exclude recommended** purged exactly those 16 (1789 → **1773**, all music intact), Add folder / Remove / actions menu / rules + library persistence across force-stop all verified, playback still starts after the lint fixes (STATE_PLAYING, position advancing), 114 unit tests + lint (0 errors) green, logcat clean, no crashes.
